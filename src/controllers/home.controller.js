...
export async function home(req, res, next) {
    try {
        ...
        const trendingTags = Array.isArray(trendingTagsAgg)
            ? trendingTagsAgg.map(t => ({
                tag: t._id,
                count: t.count
            }))
            : [];
...
export async function getPostsByTag(req, res) {
    try {
        const { tag, page = 1, limit = 12, sort = "latest" } = req.query;
        ...
        if (sort === "oldest") sortQuery = { publishedAt: 1 };
        ...
        console.error(err);
        res.status(500).json({ success: false, message: "Failed to fetch posts by tag" });
    }
}
