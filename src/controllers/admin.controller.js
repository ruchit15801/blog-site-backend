...
export async function adminPublishPostNow(req, res, next) {
    try {
        ...
        // Credit wallet if post was scheduled and is now being published (only for user posts, not admin)
        if (wasScheduled) {
            const { creditBlogPostReward } = await import('../utils/wallet.js');
            creditBlogPostReward(post._id, post.author).catch((err) => {
                console.error('Failed to credit wallet for admin-published post:', err);
            });
        }
...
export async function listPendingUserPosts(req, res, next) {
    try {
      ...
      const [posts, total] = await Promise.all([
        BlogPost.find(filter)
          ...
          .select('title slug bannerImageUrl createdAt readingTimeMinutes status scheduledAt'),
...
export async function approveUserPost(req, res, next) {
    try {
      const { id } = req.params;
      ...
      if (post.scheduledAt) {
        ...
        creditBlogPostReward(post._id, post.author).catch(console.error);
      }
...
export async function rejectUserPost(req, res, next) {
    try {
      const { id } = req.params;
